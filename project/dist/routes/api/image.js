...
image.get('/', (req, res) => {
    ...
    if (checkFileExists(fullPath)) {
        ...
        if (userQuery.filename && !userQuery.width && !userQuery.height) {
            ...
            const queryImage = path.join(rootDir, 'images', 'resized', `${fileName}_${width}x${height}.jpg`);
            if (checkFileExists(queryImage)) {
                res.sendFile(queryImage);
            }
            else {
                image_resize(path.join(rootDir, 'images', `${imageFile}`), path.join(rootDir, 'images', 'resized', `${fileName}_${width}x${height}.jpg`), width, height).then(() => {
                    res.sendFile(path.join(rootDir, 'images', 'resized', `${fileName}_${width}x${height}.jpg`));
                });
            }
        }
    }
